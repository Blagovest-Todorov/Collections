...
namespace Collections
{
    class CollectionsExamples
    {
        static void Main()
        {
            ...
            Collection<int> nums = new Collection<int>(); // We create empty collection
            Console.WriteLine($"Empty collection: {nums}");
            Console.WriteLine($"Count: {nums.Count}. Capacity: {nums.Capacity}");
            Console.WriteLine();

            nums = new Collection<int>(new int[] { 10, 20, 30, 40, 50 }); //We add numbers
            Console.WriteLine(nums);
            Console.WriteLine($"Count: {nums.Count}. Capacity: {nums.Capacity}");
            Console.WriteLine();

            //var string_nums = new Collection<string>(new string[] { "dimo", "ivan", "tonny" }); 
            ////We add numbers
